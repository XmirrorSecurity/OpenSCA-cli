...
// checkArchive 校验压缩包格式
func checkArchive(filepath string) {
	// 校验Zip包格式
	if filter.Zip(filepath) || filter.Jar(filepath) {
		...
		if index > 0 {
			os.WriteFile(filepath, data[index:], 0766)
		}
	}
}
...
func (e Engine) unArchiveFile(filepath string) (root *model.DirTree) {
	...
	temp.DoInTempDir(func(tempdir string) {
		filepath = path.Join(tempdir, path.Base(filepath))
