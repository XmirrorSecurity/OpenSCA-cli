...
func parsePackageLock(depRoot *model.DepTree, file *model.FileData) (deps []*model.DepTree) {
	deps = []*model.DepTree{}
	lock := PackageLock{}
	...
	for !q.Empty() {
		...
		for name, depmap := range depmaps {
			...
			if d, ok := depMap[name]; !ok {
				...
				dep.Name = name
				dep.Version = model.NewVersion(depmap.Version)
				depMap[name] = dep
			} else {
				newver := model.NewVersion(depmap.Version)
